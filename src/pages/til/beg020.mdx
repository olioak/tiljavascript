...
If you want to change the value of a variable in JavaScript, you do the same thing you did to **initialize** it (see code below).

Variables only store one value, so before the variable is updated, it contains the old value. After, it contains the new value.

```js
var message; // The only time we use var (declaration)
